...
///
/// Open this early (e.g. during TTS playback) so the Bluetooth HFP codec
/// switch and mic warmup happen while audio is still playing. By the time
/// you call `record_with_vad_warm`, the mic is already producing real samples.
pub struct WarmMic {
    sample_rate: u32,
...
/// Record from a pre-warmed mic with VAD auto-stop.
///
/// Like `record_with_vad` but uses an already-open mic. The mic was opened
/// earlier (e.g. during TTS playback) so the Bluetooth warmup is already done.
/// Plays a ding, then clears the buffer (discards warmup garbage) and starts
/// fresh calibration + VAD.
pub fn record_with_vad_warm(
    mut warm_mic: WarmMic,
    max_duration_ms: u64,
    ...
    // Ding to signal we're listening — mic is already open and warm
    play_ding();
    if !QUIET.load(Ordering::Relaxed) {
        eprintln!("Listening...");
        eprintln!("Recording from: default ({}Hz)", warm_mic.sample_rate);
    }

    // Clear the buffer — discard any audio captured during TTS playback / warmup
    warm_mic.buffer.lock().unwrap().clear();

    // From here the logic is identical to record_with_vad
    let adaptive_threshold = if calibration_ms > 0 {
        ...
        while cal_start.elapsed() < calibration_duration {
            if INTERRUPTED.load(Ordering::Relaxed) {
                warm_mic.stop.store(true, Ordering::SeqCst);
                if let Some(t) = warm_mic.mic_thread.take() {
                    let _ = t.join();
                }
                let samples = match Arc::try_unwrap(warm_mic.buffer) {
                    Ok(mutex) => mutex.into_inner().unwrap(),
                    Err(arc) => arc.lock().unwrap().clone(),
                };
                return Ok((samples, warm_mic.sample_rate));
            }
            let peak_bits = warm_mic.recent_peak.load(Ordering::Relaxed);
    ...
    log_recording_stats(&samples, warm_mic.sample_rate);
    maybe_save_recording(&samples, warm_mic.sample_rate);

    Ok((samples, warm_mic.sample_rate))
}
...
///
/// The mic was opened during TTS playback so Bluetooth warmup is done.
pub fn listen_and_transcribe_vad_warm(
    ...
    let (samples, sample_rate) = match record_with_vad_warm(
        warm_mic,
        max_duration_ms,
        silence_timeout_ms,
        silence_threshold,
